...
def calcular_resultados_finales(db: Session, torneo_id: int, formato: FormatoTorneo) -> dict:
    ...
    # Campeon
    campeon = None
    es_todos_contra_todos = formato == FormatoTorneo.todos_contra_todos
    
    if es_todos_contra_todos:
        tabla = tabla_posiciones_torneo(db, torneo_id)
        if tabla:
            campeon = CampeonResponse(equipo_id=tabla[0].equipo_id, equipo_nombre=tabla[0].equipo_nombre)
    else:
    ...
    if top_goleadores and top_goleadores[0].valor > 0:
        tg = top_goleadores[0]
        nombre_completo = f"{tg.usuario_nombre} {tg.usuario_apellido}".strip()
        goleador = JugadorDestacadoResponse(
    ...
    if top_vallas:
        tv = top_vallas[0]
        valla_invicta = VallaInvictaDestacadaResponse(
